 ...
 * \file      sx1276-board.c
 *
 * \brief     SX1276 board-level driver for STM32L051 + RA-01H.
 *
 *            SPI1:   SCLK=PA5, MISO=PA6, MOSI=PA7
 *            NSS:    PA4   (manual CS)
 *            RESET:  PB0
 *            DIO0:   PB1   (EXTI0_1 - TxDone / RxDone)
 *            DIO1:   PB2   (EXTI2_3 - RxTimeout)
 */
...
/* Diagnostics: incremented from IRQ context, read via debugger/RAM dump. */
...
void SX1276IoInit( void )
{
    ...
    /* DIO0 - PB1, input, no pull (EXTI configured later by SX1276IoIrqInit) */
    GpioInit( &SX1276.DIO0, PB_1, PIN_INPUT, PIN_PUSH_PULL, PIN_PULL_DOWN, 0 );
    /* DIO1 - PB2, input, no pull */
    GpioInit( &SX1276.DIO1, PB_2, PIN_INPUT, PIN_PUSH_PULL, PIN_PULL_DOWN, 0 );
...
void SX1276Reset( void )
{
    ...
    DelayMs( 1 );
    /* Release reset and keep the line actively high to avoid floating state. */
    GpioInit( &SX1276.Reset, PB_0, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
    DelayMs( 6 );
}
...
void SX1276IoIrqInit( DioIrqHandler **irqHandlers )
{
    ...
    /* DIO0 - PB1, rising edge (TxDone / RxDone) */
