...
void SX1276SetChannel( uint32_t freq )
{
    ...
    uint8_t opMode = SX1276Read( REG_OPMODE );

    /* Select HF/LF band according to target frequency.
     * AS923 must use HF mode (bit 3 cleared). */
    if( freq > RF_MID_BAND_THRESH )
    {
        opMode = ( opMode & RFLR_OPMODE_FREQMODE_ACCESS_MASK ) |
    ...
    case MODEM_LORA:
        {
            if( bandwidth > 2 )
            {
                /* Clamp invalid LoRa BW to 500 kHz to avoid hard lock. */
                bandwidth = 2;
            }
            bandwidth += 7;
...
static void SX1276SetTx( uint32_t timeout )
{
    ...
    /* Workaround: TimerStart(&TxTimeoutTimer) can deadlock in current timer port.
     * Keep TX progress driven by DIO0 TxDone IRQ for now. */
...
__weak uint8_t SX1276Read( uint32_t addr )
{
    uint8_t data;
...
static uint8_t GetFskBandwidthRegValue( uint32_t bw )
{
    ...
    /* Value not found: return the closest valid value instead of blocking forever. */
    return FskBandwidths[( sizeof( FskBandwidths ) / sizeof( FskBandwidth_t ) ) - 1].RegValue;
}
